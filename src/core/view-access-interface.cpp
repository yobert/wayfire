...
variant_t view_access_interface_t::get(const std::string & identifier, bool & error)
{
    ...
    {
        out = (_view->state().tiled_edges & WLR_EDGE_LEFT) > 0;
    } else if (identifier == "tiled-right")
    {
        out = (_view->state().tiled_edges & WLR_EDGE_RIGHT) > 0;
    } else if (identifier == "tiled-top")
    {
        out = (_view->state().tiled_edges & WLR_EDGE_TOP) > 0;
    } else if (identifier == "tiled-bottom")
    {
        out = (_view->state().tiled_edges & WLR_EDGE_BOTTOM) > 0;
    } else if (identifier == "maximized")
    {
        out = _view->state().tiled_edges == TILED_EDGES_ALL;
    } else if (identifier == "floating")
    {
        out = _view->state().tiled_edges == 0;
