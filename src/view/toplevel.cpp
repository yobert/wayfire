...
wayfire_view wf::toplevel_find_primary_view(const toplevel_sptr_t& toplevel)
{
...
void wf::toplevel_saved_geometry_t::store_geometry(const toplevel_sptr_t& tl)
{
    auto& ts = tl->current();
...
void wf::toplevel_emit_move_request(const toplevel_sptr_t& toplevel)
{
    toplevel_move_request_signal data;
    data.toplevel = toplevel;
    toplevel->current().primary_output->emit_signal(
        "toplevel-move-request", &data);
}

void wf::toplevel_emit_resize_request(const toplevel_sptr_t& toplevel,
    uint32_t edges)
{
...
static void do_minimize(const wf::toplevel_sptr_t& toplevel)
{
    auto views = wf::get_core().find_views_with_toplevel(toplevel.get());
    for (auto& v : views)
    {
...
static void do_restore(const wf::toplevel_sptr_t& toplevel)
{
    auto views = wf::get_core().find_views_with_toplevel(toplevel.get());
    for (auto& v : views)
    {
...
void wf::toplevel_emit_minimize_request(const toplevel_sptr_t& toplevel,
    bool minimized)
{
 ...
 */
static void move_to_workspace(const wf::toplevel_sptr_t& toplevel,
    ...
    uint32_t tiled_edges, std::optional<wf::point_t> ws)
{
    ...
    if (toplevel->pending().fullscreen || !wo)
    {
        return;
    }
